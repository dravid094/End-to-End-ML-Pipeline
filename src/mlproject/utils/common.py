...
def read_yaml(path_to_yaml : Path) -> ConfigBox:
    ...
    try:
        with open (open_to_yaml) as yaml_file:
...
def create_directories(path_to_directories: list, verbose=True):
    ...
    Args:
        path_to_directories(list): list of path os directories
...
def save_jason(path: Path, data: dict)
    ...
    with open(path,'w') as f:
        jason.dump(data, f, indent=4)

    logger.info(f'jason file saved at:{path}')


@ensure_annotations
def load_jason(path:path) -> ConifgBox:
    """load jason files data
